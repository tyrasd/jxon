...
function d(e){for(var t in e){if(e.hasOwnProperty(t)){return false}}return true&&JSON.stringify(e)===JSON.stringify({})}/**
     ...
     */
function m(e){return JSON.parse(JSON.stringify(e))}function p(e){if(!r.parseValues){return e}if(i.test(e)){return null}if(o.test(e)){return e.toLowerCase()==="true"}if(isFinite(e)){return parseFloat(e)}if(r.autoDate&&isFinite(Date.parse(e))){return new Date(e)}return e}function h(){}h.prototype.toString=function(){return"null"};h.prototype.valueOf=function(){return null};function g(e){return e===null?new h:e instanceof Object?e:new e.constructor(e)}function v(e,t,i,o){var a=4,l=3,s=1,f=n.length,u=e.hasChildNodes(),c=e.nodeType===e.ELEMENT_NODE&&e.hasAttributes(),d=Boolean(t&2),m=0,h="",x=d?{}:/* put here the default value for empty nodes: */r.trueIsEmpty?true:"",y,w;if(u){for(var T,S=0;S<e.childNodes.length;S++){T=e.childNodes.item(S);if(T.nodeType===a){h+=T.nodeValue}else if(T.nodeType===l){h+=T.nodeValue.trim()}else if(T.nodeType===s&&!(r.ignorePrefixedNodes&&T.prefix)){n.push(T)}}}var N=n.length,O=p(h);if(!d&&(u||c)){x=t===0?g(O):{}}for(var P=f;P<N;P++){y=n[P].nodeName;if(r.lowerCaseTags){y=y.toLowerCase()}w=v(n[P],t,i,o);if(x.hasOwnProperty(y)){if(x[y].constructor!==Array){x[y]=[x[y]]}x[y].push(w)}else{x[y]=w;m++}}if(c){var b=e.attributes.length,E=o?"":r.attrPrefix,C=o?{}:x;for(var D,j,X=0;X<b;m++,X++){D=e.attributes.item(X);j=D.name;if(r.lowerCaseTags){j=j.toLowerCase()}C[E+j]=p(D.value.trim())}if(o){if(i){Object.freeze(C)}x[r.attrKey]=C;m-=b-1}}if(t===3||(t===2||t===1&&m>0)&&h){x[r.valueKey]=O}else if(!d&&m===0&&h){x=O}if(i&&(d||m>0)){Object.freeze(x)}n.length=f;return x}function x(e,n,i){var o,a,l;if(i.constructor===String||i.constructor===Number||i.constructor===Boolean){n.appendChild(e.createTextNode(i.toString()));/* verbosity level is 0 or 1 */
